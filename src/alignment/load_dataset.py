...
def gen(shards, num_input, num_output, vocab_size, num_visual_tokens, num_action_tokens, start_frame_idx=None):
    for shard in shards:
        with open(shard, "r") as f:
            for line in f:
                instance_info = json.loads(line)
...
def get_VLA_dataset_legacy(args, vocab_size, split='train', start_frame_idx=None):
    root = args.data_root
...
def get_VLA_dataset(args, vocab_size, split='train', start_frame_idx=None):
    root = args.data_root
    file_format = 'data_bridge2_processed_{}.jsonl'
    shards = [os.path.join(root, split, file_format.format(i)) for i in range(len(os.listdir(os.path.join(root, split))))]
